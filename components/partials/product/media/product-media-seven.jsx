...
export default function ProductMediaSeven ( props ) {
    ...
    return (
        <div className={ `product-single-gallery ${ adClass }` }>
            <div className="skel-pro skel-magnifier-vertical skel-full col-12"></div>

            {
                product &&
                <>
                    <div className="product-slider-container mb-auto">
                    ...
                    <div className="prod-thumbnail thumb-vertical owl-dots order-lg-first" id='carousel-quick-dots'>
                        {
                            product && product.pictures.map( ( item, index ) => (
                                <div className="owl-dot media-with-lazy" key={ `owl-dot-${ index }` } onClick={ ( e ) => changeMediaIndex( index, e ) }>
                                    <figure><LazyLoadImage src={ process.env.NEXT_PUBLIC_ASSET_URI + item.url } alt="Thumbnail" width="100%" height="auto" /></figure>
                                </div>
                            ) )
                        }
                    </div>
